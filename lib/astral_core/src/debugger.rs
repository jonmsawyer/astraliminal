                                ...
                                .with_children(|parent| {
                                    ...
                                    parent
                                        ...
                                        .with_children(|parent| {
                                            ...
                                            parent
                                                .spawn(DebugUiIsUpsideDownBundle::new(false, None));
                                            // parent.spawn(DebugUiCharacterRotationYBundle::new(
                                            //     0, None,
                                            // ));
                                            ...
                                            parent.spawn(DebugUiCharacterLookAngleBundle::new(
                                                Vec3::ZERO,
                                                None,
                                            ));
...
fn _update_character_looking_at(
...
fn update_is_upside_down(
    ...
    for mut text in is_upside_down_query.iter_mut() {
        *text = Text::from_section(
            format!("Is Upside Down? {}", debug_data.is_upside_down,),
            DebugUiTextStyle::default(),
        );
    }
}

/// Update the Rotation Y status in the Debugger UI.
fn update_look_angle(
    mut look_angle_query: Query<&mut Text, With<DebugUiCharacterLookAngle>>,
    character_data: Res<CharacterData>,
) {
    for mut text in look_angle_query.iter_mut() {
        *text = Text::from_section(
            format!(
                "Look Angle:\n  x = {}\n  y = {}",
                character_data.look_angle.x, character_data.look_angle.y,
            ),
            DebugUiTextStyle::default(),
        );
    }
