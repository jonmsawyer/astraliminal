...
pub mod prelude {
    ...
    pub use bevy_inspector_egui::{prelude::*, quick::*};
...
impl Plugin for AstraliminalPlugins {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(
            Update,
            (asset_loaded, change_reflection_type, toggle_rotation),
        )
        .add_systems(
            Update,
            rotate_camera
...
fn setup(
    ...
    spawn_furball(&mut commands, &asset_server);
    spawn_avocado(&mut commands, &asset_server);
...
// Spawns a fur ball.
fn spawn_furball(commands: &mut Commands, asset_server: &Res<AssetServer>) {
    let _radius: f32 = 1.0;

    // Environment (see `async_colliders` example for creating colliders from scenes)
    commands.spawn((
        SceneBundle {
            scene: asset_server.load("models/Furball.glb#Scene0"),
...
// Spawns an avocado.
fn spawn_avocado(commands: &mut Commands, asset_server: &Res<AssetServer>) {
    let _radius: f32 = 1.0;

    // Environment (see `async_colliders` example for creating colliders from scenes)
    commands.spawn((
        SceneBundle {
            scene: asset_server.load("models/Avocado.glb#Scene0"),
